<template>
  <main id="bg">
    ...
    <el-dialog title="登录" :visible.sync="dialogFormVisible" width="30%">
      ...
      <el-button type="primary" @click.native.prevent="signin">登录</el-button>
    </span>
      <el-dialog
        title="选择角色"
        :visible.sync="role.showForVisble"
        width="30%"
        append-to-body>
        <el-form label-position="top" :model="frm" :rules="rules" ref="frm" style="margin-left: 140px">
          <el-form-item label="">
            <el-select v-model="role.roleCode" placeholder="请选择用户角色">
              <el-option v-for="item in role.roles" :label="item.name" :value="item.code" :key="item.code"></el-option>
            </el-select>
          </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
          <el-button type="primary" @click.native.prevent="secLogin">确认</el-button>
        </span>
      </el-dialog>
    </el-dialog>
  </main>
